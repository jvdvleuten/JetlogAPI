...
Copy-paste examples for both import flows. Every payload below is valid for
both the **External Partner API** and as a `jetlog://import?data=…` deeplink
(hence *deeplink-safe* on every heading — the two flows' requirements are
listed just below if you're wondering what that guarantees). Each request
example ends with a clickable **▶ Open this example in Jetlog** line that
carries the same encoded payload — click it to try the import live. Opening
one requires the Jetlog app; without it the link falls back to an explanation
page instead of failing silently. The response shapes under "Reading the API
response" further down have no link — there's nothing to open, they're what
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-15%22%2C%22flight_number%22%3A%22KL1024%22%2C%22from%22%3A%22EGLL%22%2C%22to%22%3A%22EHAM%22%2C%22people%22%3A%5B%7B%22ref_id%22%3A%22SELF%22%2C%22role%22%3A%22PIC%22%7D%2C%7B%22ref_id%22%3A%22FO1%22%2C%22role%22%3A%22FO%22%7D%2C%7B%22ref_id%22%3A%22CA1%22%2C%22role%22%3A%22Purser%22%7D%5D%7D%5D%2C%22people%22%3A%5B%7B%22ref_id%22%3A%22FO1%22%2C%22first_name%22%3A%22Fantas%22%2C%22last_name%22%3A%22Tico%22%2C%22default_role%22%3A%22FO%22%2C%22employee_number%22%3A%2200923%22%7D%2C%7B%22ref_id%22%3A%22CA1%22%2C%22first_name%22%3A%22Sally%22%2C%22last_name%22%3A%22Skyway%22%2C%22default_role%22%3A%22Purser%22%2C%22employee_number%22%3A%2201556%22%7D%5D%7D)
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-16%22%2C%22flight_number%22%3A%22KL1601%22%2C%22from%22%3A%22EHAM%22%2C%22to%22%3A%22LEMD%22%2C%22off_blocks%22%3A%2207%3A05%22%2C%22on_blocks%22%3A%2209%3A40%22%7D%2C%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-16%22%2C%22flight_number%22%3A%22KL1602%22%2C%22from%22%3A%22LEMD%22%2C%22to%22%3A%22EHAM%22%2C%22off_blocks%22%3A%2210%3A25%22%2C%22on_blocks%22%3A%2213%3A00%22%7D%5D%2C%22people%22%3A%5B%5D%7D)
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-14%22%2C%22flight_number%22%3A%22KL1023%22%2C%22from%22%3A%22EHAM%22%2C%22to%22%3A%22EGLL%22%2C%22remarks%22%3A%22Diverted%20to%20EGKK%20for%20weather.%22%7D%5D%2C%22people%22%3A%5B%5D%7D)
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-16%22%2C%22flight_number%22%3A%22KL1601%22%2C%22from%22%3A%22EHAM%22%2C%22to%22%3A%22LEMD%22%2C%22off_blocks%22%3Anull%7D%5D%2C%22people%22%3A%5B%5D%7D)
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-14%22%2C%22flight_number%22%3A%22KL1023%22%2C%22from%22%3A%22EHAM%22%2C%22to%22%3A%22EGLL%22%2C%22is_deleted%22%3Atrue%7D%5D%2C%22people%22%3A%5B%5D%7D)
...
[▶ Open this example in Jetlog](https://jetlog.app/import?data=%7B%22entries%22%3A%5B%7B%22type%22%3A%22flight%22%2C%22date%22%3A%222026-08-18%22%2C%22flight_number%22%3A%22KL809%22%2C%22from%22%3A%22AMS%22%2C%22to%22%3A%22KUL%22%7D%5D%2C%22people%22%3A%5B%5D%7D)
...
The custom scheme and the public HTTPS launcher accept the same `data`
parameter — a URL-encoded JSON string — and drive the same import flow:
`jetlog://import?data=<encoded>` opens the app directly; the parts of this
repo meant to be clicked (web pages, this README) use
`https://jetlog.app/import?data=<encoded>` instead, because a custom scheme
like `jetlog://` isn't clickable from a browser or a rendered Markdown page.
The app parses `?data=` the same way regardless of which one opened it.

**The encoding contract, precisely:**
- Encode the JSON **once**, with `encodeURIComponent` semantics (percent-encode
  everything outside the unreserved set; Python's `urllib.parse.quote(...,
  safe="")` below is a stricter superset of that and decodes identically). Do
  this once — never encode it a second time before appending it to the URL.
...
Get this wrong — decode twice, or apply `+`-to-space mapping — and a payload
containing a literal `%` or `+` (a common thing to type in `remarks`) silently
corrupts instead of failing loudly, which is why both the app and this repo's
validator treat it as a hard contract rather than an implementation detail.
...
print("jetlog://import?data=" + encoded)
print("https://jetlog.app/import?data=" + encoded)
'
```
